...
module.exports = {
        ...
        createServer: function createServer(options) {
                ...
                // crypto components are suffer the same malady
                if(options.key && Buffer.isBuffer(options.key)) 
                    opts.key = options.key;
                if(options.ca && Buffer.isBuffer(options.ca)) 
                    opts.ca = options.ca;
                if(options.certificate && Buffer.isBuffer(options.certificate)) 
                    opts.certificate = options.certificate;
